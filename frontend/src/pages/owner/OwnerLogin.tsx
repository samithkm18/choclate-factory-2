...
export const OwnerLogin: React.FC = () => {
    ...
    try {
      ...
      if (res.ok) {
        if (data.user.role === 'owner') {
          ...
          setError('Access denied. This dashboard is restricted to Owner credentials.');
        }
      } else {
  ...
  return (
    <div className="max-w-md mx-auto py-32 px-6 font-sans text-white">
      <div className="bg-brand-panelBg border-2 border-brand-gold/30 rounded-3xl p-8 space-y-6 shadow-2xl relative">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-serif text-brand-goldLight uppercase font-semibold">OWNER CELLAR GATE</h2>
          <p className="text-xs text-zinc-400">Restricted zone. Owner authentication token required.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-left">
          <div>
            ...
            <div className="relative">
              <input 
                ...
                required
                placeholder="e.g. owner@manis.com"
          ...
          >
            {loading ? 'Decrypting credentials...' : <><LogIn size={14} /> Unlock Dashboard</>}
          </button>
        </form>
